...
EGifOpenFileHandle(const int FileHandle, int *Error)
{
    ...
    GifFile = (GifFileType *) malloc(sizeof(GifFileType)); //targetA
    sc_marker();
    if (GifFile == NULL) {
        return NULL;
    }
...
EGifCloseFile(GifFileType *GifFile, int *ErrorCode)
{
    ...
    Private = (GifFilePrivateType *) GifFile->Private; //targetA
    sc_marker();
    if (Private == NULL)
    return GIF_ERROR;
    if (!IS_WRITEABLE(Private)) {
...
EGifSpew(GifFileType *GifFileOut) 
{
    ...
    if (EGifCloseFile(GifFileOut, NULL) == GIF_ERROR) //target
        return (GIF_ERROR);
