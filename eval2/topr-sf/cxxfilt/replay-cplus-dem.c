...
cplus_demangle (const char *mangled, int options)
{
  ...
  ret = internal_cplus_demangle (work, mangled); //target
  squangle_mop_up (work);
  return (ret);
}
...
internal_cplus_demangle (struct work_stuff *work, const char *mangled)
{
  ...
  {
    success = demangle_signature (work, &mangled, &decl); //target
  }
                    ...
                    const char **mangled, string *declp)
{
  ...
  case '5': case '6': case '7': case '8': case '9':
    ...
    success = demangle_class (work, mangled, declp); //target
    if (success)
      {
        remember_type (work, oldmangled, *mangled - oldmangled);
      }
...
demangle_class (struct work_stuff *work, const char **mangled, string *declp)
{
  ...
  btype = register_Btype (work); //target
...
void bbl_marker(int id)
{
    //log visited BBL IDs during execution
    //FILE *f1 = fopen("vbbls.txt", "a");
    //fprintf(f1, "%d\n", id);
    //fclose(f1);
    return;
}
...
register_Btype (struct work_stuff *work)
{
  ...
  ret = work -> numb++;
  work -> btypevec[ret] = NULL; //targetA
  sc_marker();
  return(ret);
}
